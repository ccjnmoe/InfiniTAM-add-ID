...
/*
_CPU_AND_GPU_CODE_ static const Vector3f colorSet[2] = {
		{ 255.0f, 255.0f, 0.0f },
		{ 255.0f, 0.0f, 255.0f }
};*/
...
_CPU_AND_GPU_CODE_ inline bool castRay(Vector3f &pt_out, int x, int y, const TVoxel *voxelData, const typename TIndex::IndexData *voxelIndex, Matrix4f invM,
	Vector4f projParams, float oneOverVoxelSize, float mu, float viewFrustum_min, float viewFrustum_max, unsigned int &ID)
{
	...
	// try add ID 
	ID = readID(voxelData, voxelIndex, pt_result, hash_found);
...
_CPU_AND_GPU_CODE_ inline void drawRendering(const bool & foundPoint, const float & angle, Vector4u & dest, unsigned int ID)
{
	Vector3f colorSet[2] ;
	colorSet[0].x = 255.0f; colorSet[0].y = 255.0f; colorSet[0].z = 0.0f;
	colorSet[1].x = 255.0f; colorSet[1].y = 0.0f; colorSet[1].z = 255.0f;
	...
	unsigned int colorjudge = ID%2;
	float outRes = 0.8f * angle + 0.2f;
	dest.x = (uchar)(outRes * colorSet[colorjudge].x);
	dest.y = (uchar)(outRes * colorSet[colorjudge].y);
	dest.z = (uchar)(outRes * colorSet[colorjudge].z);
	dest.w = 255;
...
class RaycastRenderer_GrayImage {
	...
	_CPU_AND_GPU_CODE_ inline void processPixel(int x, int y, int locId, bool foundPoint, const Vector3f & point, const Vector3f & outNormal, float angle, unsigned int ID) // try add ID to this func!!
	{
		drawRendering(foundPoint, angle, outRendering[locId], ID);//take a try
	}
};
...
class RaycastRenderer_ColourImage {
	...
	_CPU_AND_GPU_CODE_ inline void processPixel(int x, int y, int locId, bool foundPoint, const Vector3f & point, const Vector3f & outNormal, float angle, unsigned int ID)/////////////
	{
...
class RaycastRenderer_ICPMaps {
	...
	_CPU_AND_GPU_CODE_ inline void processPixel(int x, int y, int locId, bool foundPoint, const Vector3f & point, const Vector3f & outNormal, float angle, unsigned int ID)
	{
		drawRendering(foundPoint, angle, outRendering[locId],ID);/// This is the real position to draw on the screen
		if (foundPoint)
		{
			Vector4f outPoint4;
	...
	float oneOverVoxelSize, const Vector2f *minmaxdata, float mu, Vector3f lightSource)
{
	...
	bool foundPoint = castRay<TVoxel,TIndex>(pt_ray, x, y, voxelData, voxelIndex, invM, projParams, oneOverVoxelSize, mu, viewFrustum_min, viewFrustum_max, ID);

	computeNormalAndAngle<TVoxel,TIndex>(foundPoint, pt_ray, voxelData, voxelIndex, lightSource, outNormal, angle);

	renderer.processPixel(x,y, locId, foundPoint, pt_ray, outNormal, angle, ID);
}
