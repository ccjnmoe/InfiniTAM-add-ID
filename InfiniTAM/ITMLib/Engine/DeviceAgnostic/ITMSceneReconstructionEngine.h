...
_CPU_AND_GPU_CODE_ inline float computeUpdatedVoxelDepthInfo(TVoxel &voxel, const Vector4f & pt_model, const Matrix4f & M_d, const Vector4f & projParams_d,
	float mu, int maxW, const float *depth, const Vector2i & imgSize)//, unsigned int &IDplus, int &IDcount)/////addID///////
{
	...
	if (eta < -mu) return eta; // what if I jump this line ? -- Meaning less
	...
	voxel.w_depth = newW;
	//voxel.ID++;
	//voxel.ID = 1;// dosen't work here,why?
	/*IDcount++;
	if (IDcount > 2);
	{
		IDplus++;
...
struct ComputeUpdatedVoxelInfo<false,TVoxel> {
	...
	{
		computeUpdatedVoxelDepthInfo(voxel, pt_model, M_d, projParams_d, mu, maxW, depth, imgSize_d);//,IDplus,IDcount);
	}
};
